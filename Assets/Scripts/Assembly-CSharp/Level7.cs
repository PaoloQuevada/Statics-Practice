...
public class Level7 : MonoBehaviour, IDataPersistence
{
	...
	private void Awake()
	{
		...
		details.text = "Point A is " + point1Segments[0] + " meters away from the origin in the x direction, " + point1Segments[1] + " meters away from the origin in the y direction, " + point1Segments[2] + " meters away from the origin in the z direction. \nPoint B is " + point2Segments[0] + " meters away from the origin in the x direction, " + point2Segments[1] + " meters away from the origin in the y direction, " + point2Segments[2] + " meters away from the origin in the z direction. ";
	...
	{
		if (string.IsNullOrWhiteSpace(length.text) | string.IsNullOrWhiteSpace(alpha.text) | string.IsNullOrWhiteSpace(beta.text) | string.IsNullOrWhiteSpace(gamma.text))
		{
			wrong.text = "Please input an answer!";
		}
		else if ((Convert.ToDouble(length.text) == lengthAnswer) & (Convert.ToDouble(alpha.text) == alphaAnswer) & (Convert.ToDouble(beta.text) == betaAnswer) & (Convert.ToDouble(gamma.text) == gammaAnswer))
		{
			attempts++;
			winCount++;
